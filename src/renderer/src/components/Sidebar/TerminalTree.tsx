...
export function TerminalTree(): React.JSX.Element {
  ...
  const { getGroupMonitoringState, getTerminalMonitoringState } = useMonitoring()
    ...
    <div className="terminal-tree" onClick={closeContextMenu}>
      ...
      <ul className="terminal-tree__list" role="tree" aria-label="Terminal group list">
        {groups.map((group) => {
          ...
          const groupMonitoringState = getGroupMonitoringState(group.id)

          return (
            <li key={group.id} role="treeitem" aria-expanded={!group.collapsed} aria-level={1}>
              <div
                className={`terminal-tree__group${isActive ? ' terminal-tree__group--active' : ''}${groupMonitoringState.hasAttention ? ' terminal-tree__group--attention' : ''}${dragOverGroupId === group.id ? ' terminal-tree__group--dragover' : ''}`}
                onClick={(e) => { e.stopPropagation(); closeContextMenu(); switchGroup(group.id) }}
                onContextMenu={(e) => handleGroupContextMenu(e, group)}
                onDragOver={(e) => handleGroupDragOver(e, group.id)}
                onDragLeave={handleGroupDragLeave}
                onDrop={(e) => handleGroupDrop(e, group.id)}
                title={groupMonitoringState.title ?? undefined}
              >
                <span
                ...
                {groupMonitoringState.hasAttention && (
                  <span
                    className="terminal-tree__group-attention-badge"
                    aria-label={`${groupMonitoringState.affectedTerminalCount} terminals need attention`}
                  >
                    {groupMonitoringState.affectedTerminalCount}
                  </span>
                )}
              ...
              {!group.collapsed && (
                <ul className="terminal-tree__terminals" role="group">
                  {groupTerminals.map((terminal, idx) => {
                    const terminalMonitoringState = getTerminalMonitoringState(terminal.id)
                    return (
                      <li
                        key={terminal.id}
                        role="treeitem"
                        ...
                        className={`terminal-tree__item${activeTerminalId === terminal.id && isActive ? ' terminal-tree__item--active' : ''}${terminalMonitoringState?.hasAttention ? ' terminal-tree__item--attention' : ''}`}
                        draggable
                      ...
                      >
                        ...
                        {terminalMonitoringState?.hasAttention && (
                          <span
                            className={`terminal-tree__item-indicator terminal-tree__item-indicator--${terminalMonitoringState.tone}`}
                            title={terminalMonitoringState.title}
                            aria-label={terminalMonitoringState.title}
                          >
                            !
                          </span>
                        )}
